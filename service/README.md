...
- Uploaded originals are private staff-only objects. Each preview is separately decoded, orientation-normalized, resized, and re-encoded as JPEG without EXIF, with its source record, transform, SHA-256, generation, MIME type, byte size, visibility, and workflow epoch retained as provenance metadata. The upload idempotency key plus current workflow epoch make an accepted retry return the same pair; conflicting bytes or model consent fail with an idempotency conflict. The staff browser selects the latest complete current-epoch pair, while an armed ordinary-intake command freezes its triggering pair into the event before queueing; retained evidence from an earlier reset or a later upload cannot be sent to the model by substitution.
...
`GET /api/v1/healthz` exposes the active analyst, model, inventory mode, inventory URL and gateway configuration status, bounded inventory timeout, live inventory readiness, relay, and evidence-store modes so a fixture path cannot be mistaken for the canonical live path. In HTTP mode it performs a bounded `/api/v1/healthz` call to the simulator and validates the `SIMULATED` header plus the exact typed fixture envelope. It returns HTTP 503 if that probe fails or if the configured evidence store becomes unavailable. GCS-mode startup also checks that the configured bucket exists and is reachable. `/healthz` remains an equivalent container-local compatibility route, while external Cloud Run checks use the namespaced path to avoid the Google Frontend 404 observed for the exact root path on the deployed service. Inventory transport and contract failures fail the analysis operation closed rather than being silently replaced with fixture rows.
...
| `POST /api/v1/admin/passports/{id}/outbox/reconcile` | With `X-Found-Roll-Admin-Token`, republish at most 25 `PENDING` or `FAILED/PUBLISH` analysis commands for one armed ordinary intake. It reuses the deterministic task name, cannot add a custody event, and excludes `FAILED/EXECUTE`. |
...
| `GET /api/v1/judge-walkthrough` | Public, no-store, non-mutating projection of the fixed closed synthetic case. It returns only a redacted timeline, bounded-analyst metadata, and internal manifest-consistency summary; it never returns credentials, claimant evidence, restricted media, task bodies, raw actor IDs, idempotency keys, or model trace IDs. |
| `POST /api/v1/intakes` | With both demo and staff credentials, run the pre-intake decision. Sensitive or dangerous categories return tenant-specific specialist/no-upload guidance, create no record, and call no model. An accepted ordinary intake is the only route that arms the later server-side auto-queue. |
| `POST /api/v1/passports/{id}/analysis-jobs` | Retained only for deliberately unarmed prepared canonical/backward-compatible cases. It rejects armed ordinary intakes; for eligible unarmed cases it commits `EVIDENCE_READY` then `ANALYZING` with an opaque outbox command. |
| `POST /api/v1/staff/passports/{id}/evidence` | Accept one staff-authenticated multipart JPEG/PNG plus an idempotency key, retain its private original, and create a provenance-linked EXIF-free preview for the current workflow epoch. `authorize_preview_for_model=true` is an explicit model-visibility decision for the preview only. For an armed ordinary intake, the service freezes that exact authorized pair, creates one bounded analysis command, and returns its receipt. Exact pair retries resume that command; changed bytes or consent conflict. |
...
| `GET /api/v1/passports/{id}/claim-link` | Inspect the link supplied in `X-Found-Roll-Claim-Link` without returning the raw token, with `Cache-Control: no-store, private`. |
...
Rich read routes include `GET /api/v1/passports`, `/api/v1/passports/{id}`, `/events`, `/candidates`, `/manifest`, and `/api/v1/demo/snapshot`. They require the staff credential in production; they are neither claimant routes nor unauthenticated public endpoints. The sole public case read is `GET /api/v1/judge-walkthrough`, a fixed redacted synthetic projection with no credentials or mutation authority. Candidate serializers still exclude restricted fields. The claimant never receives those shapes: the one-time link routes return a purpose-built coarse projection containing only the proof question, limited case copy, link lifecycle, and attempt state.
...
| `FOUND_ROLL_ANALYST_WALL_CLOCK_TIMEOUT_SECONDS` | Whole live ADK trajectory ceiling. Production requires 240 seconds so application code can terminalize before the Cloud Run request deadline. |
...
| `FOUND_ROLL_TASK_DISPATCH_DEADLINE_SECONDS` | Per-task HTTP dispatch deadline. Production requires 305 seconds, above the 240-second analyst ceiling and 300-second Cloud Run request timeout. |
...
The live adapter is pinned to `google-adk==2.8.0` and `google-genai==2.20.0` and uses `LlmAgent`, a typed Pydantic `output_schema`, `Runner.run_async`, an ephemeral `InMemorySessionService`, `RunConfig(max_llm_calls=6)`, and a 240-second application wall-clock timeout around the complete trajectory. Its exact instruction and structured request builder are isolated in `app/agent_contract.py` as `found-roll-case-analyst-prompt-v3`; the typed proposal is `found-roll-analysis-proposal-v2`, and deterministic custody policy is `found-roll-release-v1`. The v3 instruction preserves the custody engine's frozen candidate order and makes one bounded agent decision explicit: after the authorized searches and policy-ranked candidate load, it may either request one allowed private discriminator or abstain to manual review with one allowlisted reason. It cannot select or reorder a candidate, accept a claim, mutate custody, or access an expected answer. Canonical successful trajectories remain the two authorized custodian searches followed by load, observations, and discriminator, all successful and exactly once; deviations fail closed. An abstention has its own exact search, load, and `request_manual_review` path, completes the analysis outbox, and never creates a private question. Health and snapshots expose non-secret contract identifiers so canonical receipts can match them to the submitted source hashes. The persisted model trace is the single ADK `Event.invocation_id`; the invocation count is the number of unique, non-streaming ADK events carrying `usage_metadata`, so it represents logical model calls rather than internal SDK HTTP retries. Every persisted tool outcome is reduced to a tool name plus `success`, `denied`, `abstained`, or `unavailable`, and is accepted only when its ADK response ID and name match exactly one observed call. Arguments, response bodies, token usage, prompts, and model response text are never persisted as execution evidence. Request-completion logs can expose a source-controlled allowlist of analyst failure codes for diagnosis, but never exception messages, model content, tool arguments, or runtime-supplied codes; durable outbox records keep only the coarse failure class. It sets ADK's current `GOOGLE_GENAI_USE_ENTERPRISE=TRUE` selector and the pinned Gen AI SDK's `GOOGLE_GENAI_USE_VERTEXAI=true` selector before the run. Authorized GCS previews are supplied with `google.genai.types.Part.from_uri`; staff-only originals, non-GCS records, and the restricted expected answer are omitted. The evidence boundary additionally pins `google-cloud-storage==3.13.1`, `Pillow==12.1.1`, and `python-multipart==0.0.32`. Firestore mutation bundles transact the case version, event, idempotency receipt, and any outbox/handoff/token records together. Cloud Tasks uses a deterministic task name, 305-second deadline, and OIDC service account; the app then verifies the Google-signed ID token again at `/tasks/outbox`.
