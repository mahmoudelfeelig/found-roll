...
Original and derived images are different objects. The derived path strips EXIF and any unnecessary metadata. Every crop, blur, or annotation records its source object generation and SHA-256. The application never overwrites an original with a derivative or grants the model bucket-wide browsing. Evidence ingestion is idempotent for an exact retry and scoped to the current workflow epoch. The staff preview selects the latest complete original/derivative pair from that epoch; when an ordinary intake authorizes a preview, the server freezes that exact authorized pair for its model command. Retained objects from an earlier reset or a later upload cannot be substituted as active model evidence. If the current pair is unavailable, the workspace reports that state rather than showing unrelated fixture media.
...
The persisted ordinary-intake auto-queue arm defaults to `false` and is set only after the combined demo-and-staff intake boundary accepts a non-specialist item. Staff evidence upload alone cannot arm a pre-existing case. Even an armed case remains `RECEIVED` unless its same workflow epoch has a complete original plus `MODEL_AUTHORIZED` derived preview; the service freezes the triggering pair's immutable IDs and hashes into `EVIDENCE_PACKET_READY` before committing the one analysis outbox command. A later upload cannot substitute its pair, resume the first pair, or use the public manual-analysis route. Sensitive, dangerous, unarmed, stale-epoch, staff-only-preview, and post-analysis cases cannot enqueue work through this path.
