...
The scenarios are synthetic and implementation-shaped. Passing all 15 means the exercised local policy, state, API, idempotency, fixture-agent, and privacy assertions held for this frozen matrix. It does not mean candidate matching has 100% real-world accuracy.
...
| FR-008 | Deterministic custody policy selected the canonical fixture candidate; the analyst abstained from claim acceptance, emitted a non-leading question, and preserved the twelve-call plus 2,048-output-token ADK caps | PASS |
